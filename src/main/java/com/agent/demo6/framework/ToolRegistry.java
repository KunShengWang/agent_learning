    ...
    public void registerToolsFromClass(Class<?> toolClass) {
        for (Method method : toolClass.getDeclaredMethods()) {
            ...
            if (!Modifier.isStatic(method.getModifiers())) {
                throw new IllegalArgumentException("@AgentTool 方法必须是 static：" + method.getName());
            }
            if (method.getParameterCount() != 1 || method.getParameterTypes()[0] != String.class) {
                throw new IllegalArgumentException("@AgentTool 方法必须接收一个 String argumentsJson：" + method.getName());
            }
            if (method.getReturnType() != String.class) {
                throw new IllegalArgumentException("@AgentTool 方法必须返回 String resultJson：" + method.getName());
            }
